...
#본문 섹션 (2~4개, 기능 특성에 맞게 소제목·구성 직접 결정)
소제목:
핵심 내용:
#아웃트로
기능 활용 권유 방향:
...
연관 키워드:
📌 콘텐츠 흐름
※ 개요는 간결하게. 각 Tip의 핵심 내용은 3개 불릿 이내로만 작성할 것.
#인트로
독자가 공감할 실무 문제 상황:
#기능 소개
소제목 (예: "[기능명]이란?", "[기능명], 어떻게 작동하나요?"):
핵심 내용 (2개 이내): 이 기능이 무엇인지, 어떤 문제를 해결하는지 간략히
#Tip1
소제목:
핵심 내용 (3개 이내):
#Tip2
...
function buildCtx() {
  ...
  // F/G 타이틀 자동 고정
  if ((cat==='F'||cat==='G') && formVals['ym']) {
    const ym = formVals['ym'].trim();
    // "2026년 4월" → "2026년 04월"
    const formatted = ym.replace(/(\d+)년\s*(\d+)월/, (_,y,m)=>`${y}년 ${m.padStart(2,'0')}월`);
    const suffix = cat==='F' ? '크리마 뉴스' : '릴리즈 노트';
    s+=`타이틀: ${formatted} ${suffix}\n\n`;
  }
  flds.forEach(f=>{
...
1. 제목(# H1)과 소제목(## H2, ### H3)은 한글 20자 이내로 자연스럽게 압축해줘. H1 제목은 앞 10자 이내에 핵심 키워드가 위치하도록 구성할 것(예: "리뷰 전환율 높이는 3가지 방법"처럼 키워드가 앞에). 제목에 연도("2025년") 또는 숫자("3가지")가 포함되면 유지할 것. 단, ## ### 기호는 반드시 유지할 것
2. **볼드**, --- 구분선, - 리스트 등 마크다운 형식은 원본 그대로 유지
3. 이모지는 본문에서 모두 제거할 것 (제목 포함)
4. 화자, 인칭, 내용, 구조는 절대 바꾸지 말 것
5. 본문 어투는 원본 해요체(~해요, ~있어요)를 기본으로 유지하되, 핵심 인사이트나 결론 문장에서는 합쇼체(~합니다, ~됩니다)를 자연스럽게 섞을 것. 전체의 70~80%는 해요체, 20~30%는 합쇼체 비율로. 담담하고 전문적인 어조를 유지할 것
...
const VAR_DESCS = {
  ...
  brunch: '해요체 기본 + 합쇼체 혼용 · 고민과 통찰 중심 · 담담한 전문가 어조',
